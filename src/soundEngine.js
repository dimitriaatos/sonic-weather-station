...
function toneFunc() {
  ...
  api.update((data, prevData, interval) => {
    ...
    let barometerPct = ((data.data.barometer - 1000) / 30) * 100;
    let barometerPrevPct = ((prevData.data.barometer - 1000) / 30) * 100;
    let barometerPctDiff = barometerPct - barometerPrevPct;
    console.log(barometerPctDiff);
    let relativeHumidityPct = (data.data.relativeHumidity);
    let relativeHumidityPrevPct = (prevData.data.relativeHumidity);
    ...
    let rain = data.data.rain + prevData.data.rain 
    if (rain > 0) { 
      l2filterLFO.set ({max: 1000,
      min: 191})
    }
    ...
    let l4MovingFilterPct1 = (500 + (2000 -500) * (relativeHumidityPct/100)) * 5
    let l4MovingFilterPrevPct1 = (500 + (2000 -500) * (relativeHumidityPrevPct/100))
    let l4MovingFilterPct2 = (500 + (4000 -500) * (relativeHumidityPct/100) ) * 5
    let l4MovingFilterPrevPct2 = (500 + (4000 -500) * (relativeHumidityPrevPct/100))
...
console.log(dummySig.value);
 if (relativeHumidityPctDiff > 0 ){
  l4MovingFilter3.frequency.rampTo(randomRange(100, 300) +l4MovingFilter3.frequency.value, interval/1000);
  l4MovingFilter4.frequency.rampTo(randomRange(150, 450) + l4MovingFilter4.frequency.value, interval/1000);

dummySig.rampTo(randomRange( 0 , 300) / 1000 + dummySig.value ,interval/1000 );
 }
 else
 {
  l4MovingFilter3.frequency.rampTo(l4MovingFilter3.frequency.value - randomRange(100, 300), interval/1000);
  l4MovingFilter4.frequency.rampTo(l4MovingFilter4.frequency.value - randomRange(100, 300), interval/1000);
  dummySig.rampTo(dummySig.value - (randomRange( 0 , 300) / 1000) ,interval/1000);
 }
    ...
    movingDetuneFilter.frequency.rampTo(filterDetuneSynthPct + movingDetuneFilter.frequency.value, interval / 1000);
    l3MovingFilter.frequency.rampTo(l3MovingFilterPct + l3MovingFilter.frequency.value, interval/1000)
...
export { init, toneFunc, stopFunc, changeVol, volArray, triggerRamps, handleMouseMove }
